...
const sections = [
  {
    ...
    items: [
      { label: 'Feed', icon: Rss },
      { label: 'Mapa Orbital', icon: Globe2, live: true },
          ...
          {sections.map((section) => (
            <div key={section.title}>
              ...
              <ul className="flex flex-col gap-1">
                {section.items.map(({ label, icon: Icon, live }) => (
                  <li key={label}>
                    <button
                    ...
                    >
                      ...
                      {live && (
                        <span className="flex items-center gap-1 rounded-full border border-aurora/40 bg-aurora/10 px-2 py-0.5 font-mono text-[9px] font-bold uppercase tracking-widest text-aurora">
